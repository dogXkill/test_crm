...
get_all_emails_to_check("2");
get_all_emails_to_check("3");

function get_all_emails_to_check($crm_case_check){

  if($crm_case_check == "1"){$crm_check_vst = " crm_outbox_check = '1' AND crm_inbox_check = '1' "; $crm_outbox_check = "1";}
  if($crm_case_check == "2"){$crm_check_vst = " crm_outbox_check = '0' AND crm_inbox_check = '1' "; $crm_outbox_check = "0";}
  if($crm_case_check == "3"){$crm_check_vst = " crm_outbox_check = '1' AND crm_inbox_check = '0' "; $crm_outbox_check = "1";}

    //получаем список действующих пользователей
    $users_q = "SELECT * FROM crm_members WHERE deleted <> '1'";
    $users= mysql_query($users_q);

    while($r = mysql_fetch_assoc($users)){

    $mails_to_check_q = "SELECT * FROM crm_member_emails WHERE $crm_check_vst";
...
//проставляем в таблицу с емейлами дату последней проверки почты и последний UID увиденный в почтовом ящике
...
//получаем список емейлов для дальнейшей проверки
...
function get_last_uid($email_id, $crm_outbox_check){
    ...
    $last_uid = $last_uid["server_uid"];
         ...
         //echo $get_last_uid_q;
    echo "<br><br><i>Осуществлена проверка максимального значения UID сообщения <b><u>$email_id</u></b> в таблице $select_table, полученное значение = <b>$last_uid</b></i><br><br>";
...
//делаем mbox, где по факту хранятся все сообщения глобальной, чтобы была доступна из др функций
    ...
    echo "<br><br>Проверка писем в ящике ID - <b>$email_id</b>, UID c <b>$crm_mail_last_uid</b> по <b>$num_msg</b><br><br>";
...
function inbox_outbox_adapt($crm_imap_host){
    ...
    if($crm_imap_host == "imap.yandex.ru:993/imap/ssl"){$folder_check = "Sent";}
    if($crm_imap_host == "imap.mastermail.ru:993/imap/ssl"){$folder_check = "&BB4EQgQ,BEAEMAQyBDsENQQ9BD0ESwQ1-";}
