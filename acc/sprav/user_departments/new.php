...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <body>
    ...
    <table align=center width="1200" cellspacing="0" cellpadding="5" border="0" bgcolor="#F6F6F6">
      <tbody>
        <tr class="tab_query_tit"><td align="center"><a class="sublink" href="/acc/sprav/user_departments/">Список отделов</a></td></tr>
      </tbody>
    </table>
        ...
        <td align="center">
            ...
            <table cellspacing="3" cellpadding="4" border="0">
              <tbody>
                <tr>
                  ...
                  <td><input name="department_name" id="department-name" type="text" size="30" value=""/></td>
                </tr>
                <tr>
                  <td align="right">Сортировка:</td>
                  <td><span class="err">*</span></td>
                  <td><input name="department_sort" id="department-sort" type="text" size="30" value=""/></td>
                </tr>
                <tr>
                  ...
                  <td>
                    <select name="department_submission" id="department-submission">
                      ...
                      foreach ($other_departments as $key => $value) {
                        ?>
                        <option data-id="<?=$value['id']?>" <?=$selected?>><?=$value['name']?></option>
                        <?
                      }
                      ?>
                    </select>
          ...
          <table>
            <tbody>
              <tr>
                <td align="center">
                  <button class="users_frm_butt" onclick="save_department()">Создать</button>
                </td>
                <td><span></span></td>
                <td>
    ...
    <script type="text/javascript">
      ...
      function save_department() {
        ...
        var sort = Number(document.getElementById('department-sort').value);
        if (sort == null || sort == '') {
        ...
        var submission = document.getElementById('department-submission').selectedOptions[0].attributes[0].value;
        var is_division = document.getElementById('is_division').selectedOptions[0].attributes[0].value;

        var data = {new: 1, name: name, sort: sort, submission: submission, is_division: is_division};
        console.log(data);
        if (errors == '' || errors == null) {
          $.ajax({
            url: 'handler.php',
            method: 'POST',
            data: {new: 1, name: name, sort: sort, submission: submission, is_division: is_division},
            dataType: 'html',
            async: false,
