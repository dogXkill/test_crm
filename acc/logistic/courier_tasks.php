...
//$form_of_payment = $ord_row["form_of_payment"];
      ...
      if($result = mysql_query($sql))
      {
		...
		//1
			...
			$sql = "SELECT obj_ac.nn AS number, `name`, obj_ac.num AS col, obj_ac.art_num AS art_num ,pl.uid,pl.num_sklad AS n_cl FROM obj_accounts AS obj_ac  LEFT JOIN plan_arts AS pl ON pl.art_id =obj_ac.art_num WHERE obj_ac.query_id = " . $query_id_load . " ORDER BY number;";
			//echo $sql;
			if($res = mysql_query($sql))
            {
              ...
              if($row["art_num"]!=="d"){
				...
				if (isset($row['n_cl'])){
					$row['num_col']=$row['n_cl'];
				}else{
					$row['num_col']=0;
				}
				//$row['num_col']=mb_detect_encoding($row['n_cl']);
				$row['num_col']=iconv('utf-8//IGNORE', 'windows-1251//IGNORE', $row['num_col']);
...
$get_order_info = "SELECT uid AS query_id,typ_ord, prdm_sum_acc AS prdm_sum_acc, form_of_payment AS form_of_payment, deliv_id AS deliv_id, prdm_dolg AS prdm_dolg, prdm_opl AS prdm_opl, note AS note FROM queries WHERE uid = '$query_id'";
...
$row["typ_ord"]=$ord_row["typ_ord"];
	 if($row["deliv_address"]){ $row["address"] = $row["deliv_address"];}
            ...
            $sql = "SELECT nn AS number, `name`, num AS col, art_num AS art_num FROM obj_accounts WHERE query_id = " .$temp_id. " ORDER BY nn";
