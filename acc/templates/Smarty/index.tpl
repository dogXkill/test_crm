...
<table>
<tr><td><strong>исполнитель:</strong>&nbsp;<select type="text" name="courier_id" id="courier_id" class="vod_select">{foreach from=$couriers item=i}<option value="{$i.id}">{$i.name}</option>{/foreach}
<option value="all">на всех</option>
</select>
<strong>дата поездки:</strong> <input type="text" name="date" value="{$date}" size="10" class="date_select" id="date" />
...
<b>Цель:</b> <a href="/acc/logistic/courier_tasks.php?id={$item.id}" target=_blank><strong>{$item.text}</strong></a><br/><br>

{if $item.query_id}
Заказ #: <b><a href="/acc/query/query_send.php?show={$item.query_id}" target_blank>{$item.query_id}</a></b><br>
...
{if ($item.form_of_payment == "1") }наличные{/if}
{if ($item.form_of_payment == "2") }безнал по счету{/if}
{if ($item.form_of_payment == "3") }безнал по квитанции{/if}
{if ($item.form_of_payment == "4") }прочая{/if}</span>
<br>
{else}
<b>Нет привязки к заказу</b>
{/if}

<br>Сумма к получению: <b>{$item.cash_payment} р.</b><br>
Вознаграждение водителю: <b>{$item.opl_voditel} р.</b>
<br>
<br>
{if $item.first_point}Первая точка, успеть до {$item.first_point}<br><br>{/if}


<b>Адрес:</b> <a href="http://maps.yandex.ru/?text={$item.address}" target=_blank><img src="../i/search.png"  border="0" alt="" valign="middle"></a> {$item.address}<br/>
<b>Контактное лицо:</b> {$item.contact_name}<br/>
<b>Контактные телефоны:</b> {$item.contact_phone}<br/>
{if $item.comment ne ""}<strong><b>Примечания</b>:</strong> {$item.comment}<br/>{/if}
<b>Инициатор:</b> {$item.user}<br/>


</td>
</tr>
<tr><td width="35%"><a href="/acc/logistic/courier_tasks.php?id={$item.id}"><img width="20" height="20" src="../i/lupa.gif" onmouseover="Tip('Просмотреть / редактировать')" /></a>&nbsp;<a href="/acc/logistic/courier_tasks.php?del={$item.id}&r=1" onClick="return confirm('Вы уверены, что хотите удалить заявку?');"><img width="20" height="20" src="../i/del.gif" onmouseover="Tip('Удалить')" /></a></td>
...
<strong>дата поездки:</strong> <input type="text"  value="{$item.date}" size="10" disabled/></td></tr></table>
</td>
{if $item.num eq 2}
</tr>
{/if}
...
<table>
<tr><td><strong>исполнитель:</strong>&nbsp;<select type="text" name="courier_id">{foreach from=$couriers item=i}<option value="{$i.id}">{$i.name}</option>{/foreach}</select> <strong>дата поездки:</strong> <input type="text" name="date" value="{$date}" size="10" id="date" /><a href="#" id="button1" onClick="return false;"><img src="../i/calendar.gif" alt="Календарь" border="0" /></a> <input type="submit" value="Сформировать путевой лист" class="button_big" /></td></tr>
</table>
</form>
{literal}
<script type="text/javascript">
