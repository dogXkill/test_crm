...
//дл€ timetable report
$vstavka = " AND j.cur_time LIKE '".$year."-".$month."-%' AND j.num_sotr='".$num_sotr."'  AND a.num_ord=j.num_ord ";
$items_on_page = 10000;
}else{
if ($year){$vst_date = " AND j.cur_time LIKE '".$year."-%'";}
if ($month && $year){$vst_date = " AND j.cur_time LIKE '".$year."-".$month."-%'";}
if ($month && !$year){$vst_date = " AND j.cur_time LIKE '".date("Y")."-".$month."-%'";}
if ($job_id){$vst_job = " AND j.job='".$job_id."'";}
if ($type){$vst_type = " AND a.izd_type='".$type."'";}
if (is_numeric($art_num)){$vst_art_num = " AND a.art_num='".$art_num."'";}
if (is_numeric($num_order)){$vst_num_ord = " AND j.num_ord='".$num_order."'";}
if (is_numeric($num_sotr)){$vst_num_sotr = " AND j.num_sotr='".$num_sotr."'";}
$vstavka = $vst_date." ".$vst_art_num." ".$vst_num_ord." ".$vst_num_sotr." ".$vst_job." ".$vst_type."  AND a.num_ord=j.num_ord ";
}



$select_txt = "SELECT j.j_uid, j.num_sotr, j.num_ord, j.job, j.num_of_work, j.cur_time, j.nadomn, j.order_price, a.izd_type, a.uid, a.art_id FROM job AS j, applications AS a WHERE 1 ".$vstavka." GROUP BY j.j_uid ORDER BY ".$orderby." ".$order." LIMIT 0, ".$items_on_page;
