...
$select_txt = "SELECT job.uid, job.num_sotr, job.num_ord, job.job, job.num_of_work, job.cur_time, job.nadomn, job.order_price, applications.izd_type, applications.uid FROM job, applications WHERE 1 ".$vstavka." GROUP BY job.uid ORDER BY ".$orderby." ".$order." LIMIT 0, ".$items_on_page;
...
//echo $nords_vst;
$q = "SELECT num_ord, rate, rate_lamin, rate_tigel_pril, rate_tigel_udar, rate_tisn_pril, rate_tisn_udar, rate_vstavka_dna_bok, rate_line_truba_pril, rate_line_truba_prokat, rate_line_dno_pril, rate_line_dno_prokat,
rate_upak, rate_podgotovka_truby, rate_drugoe, title FROM applications WHERE 1 $nords_vst";
//echo $q;
$rate_arr = array();
$sel = mysql_query($q);
while($select_job = mysql_fetch_array($sel)){
...
$job = $r[3];
$order_price =$r[7];
if ($job == "1"){$job_name = "ламинаци€"; if($order_price>0){$price = $order_price;}else{$price = $rate_arr[$r[2]][1];}}
if ($job == "2"){$job_name = "вырубка";  if($order_price>0){$price = $order_price;}else{$price = $rate_arr[$r[2]][3];}}
if ($job == "3"){$job_name = "тиснение"; if($order_price>0){$price = $order_price;}else{$price = $rate_arr[$r[2]][5];}}
if ($job == "4"){$job_name = "сборка"; if($order_price>0){$price = $order_price;}else{$price = $rate_arr[$r[2]][0];}}
if ($job == "5"){$job_name = "труба на линии"; if($order_price>0){$price = $order_price;}else{$price = $rate_arr[$r[2]][8];}}
if ($job == "6"){$job_name = "дно на линии"; if($order_price>0){$price = $order_price;}else{$price = $rate_arr[$r[2]][10];}}
if ($job == "7"){$job_name = "приладка вырубки"; if($order_price>0){$price = $order_price;}else{$price = $rate_arr[$r[2]][2];}}
if ($job == "8"){$job_name = "приладка тиснени€"; if($order_price>0){$price = $order_price;}else{$price = $rate_arr[$r[2]][4];}}
if ($job == "9"){$job_name = "приладка на линии (труба)"; if($order_price>0){$price = $order_price;}else{$price = $rate_arr[$r[2]][7];}}
if ($job == "10"){$job_name = "приладка на линии (дно)"; if($order_price>0){$price = $order_price;}else{$price = $rate_arr[$r[2]][9];}}
if ($job == "11"){$job_name = "упаковка"; if($order_price>0){$price = $order_price;}else{$price = $rate_arr[$r[2]][11];}}
if ($job == "12"){$job_name = "вставка дна и боковин"; if($order_price>0){$price = $order_price;}else{$price = $rate_arr[$r[2]][6];}}
if ($job == "13"){$job_name = "ручна€ подготовка трубы"; if($order_price>0){$price = $order_price;}else{$price = $rate_arr[$r[2]][12];}}
if ($job == "14"){$job_name = "выдача надомнику"; if($r[8] == "4"){$price = "0.50";}else{$price = "0.15";}}
if ($job == "15"){$job_name = "ручки с клипсами (комплект)"; $price = "0.17";}
if ($job == "16"){$job_name = "другое"; if($order_price>0){$price = $order_price;}else{$price = $rate_arr[$r[2]][13];}}
...
<table width="1100" border="0" cellpadding="5" cellspacing="1">
...
<td>
<?if ($_GET["num_sotr"] or $_GET["art_num"] or $_GET["year"] or $_GET["year"] or $_GET["month"] or $_GET["num_ord"] or $_GET["order"] or $_GET["type"]) { ?><a href="exp_html.php">сбросить</a><?}?>
<!--<img src="../../../i/planner.png" width="16" height="16" alt="" style="vertical-align:middle; cursor:pointer;" onclick="hide_show_query()">-->
<i class="fa-duotone fa-square-terminal icon_btn_r21 icon_planner" onmouseover="Tip('Посмотреть запрос sql');" style="vertical-align:middle; cursor:pointer;" onclick="hide_show_query()"></i></td>
</tr></table>
<div style="display:none; font_size:8px; font-style:italic; width:1100px" id=mysql_q><br><?echo $q."<br><br>". $select_txt."<br><br>";?></div>

<?
$header = "<table border=1 width=1100><tr><td class=tab_query_tit align=center># сотр</td><td class=tab_query_tit width=150>им€</td><td class=tab_query_tit width=50>тип издели€</td><td class=tab_query_tit align=center>заказ</td><td class=tab_query_tit>номер за€вки</td><td class=tab_query_tit>id этапа</td><td class=tab_query_tit>название этапа</td><td class=tab_query_tit>количество</td><td class=tab_query_tit>цена</td><td class=tab_query_tit align=center>стоимость</td><td class=tab_query_tit align=center>надомн</td><td class=tab_query_tit align=center>".sort_link("врем€", $order, "cur_time")."</td><td class=tab_query_tit> </td></tr>";
