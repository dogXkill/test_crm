...
<td class="table_title" style="width:100px">Сделка <img src="../../i/refresh.png" width="16" height="16" alt="" onclick="get_full_sdelka('cost')" style="cursor:pointer;"> <input type="text" size=2 value="0" id="save_sdelka_inp">/></td>
...
<td class=name><a href="/acc/users/users.php?edit=<?=$us['uid'];?>&oper=edit"><?=$fio;?></a>
...
<?if($nachisleno > $oklad*1.1){echo $warning_1;}?>
...
<span style="white-space: nowrap;word-wrap: normal;"><img src="../../i/refresh.png" width="16" height="16" alt="" onclick="get_sdelka('<?=$job_id;?>', 'cost')" id="load_img_<?=$job_id;?>" style="cursor:pointer;">
<a href="../count/exp_html.php?year_num=<?=$year;?>&month_num=<?=$month;?>&num_sotr=<?=$job_id;?>&items_on_page=500" target="_blank"><img src="../../../i/table.png" width="16" height="16" alt="" id="table_sdelka_link_<?=$job_id;?>" style="display: <? if($sdelka > "0"){?>display: inline;<?}else{?>none<?}?>"></a>
</span>
</nobr>
</td>
...
function get_full_sdelka(type_act){
job_ids = '<?=$job_ids_no_spec;?>';
jobid = job_ids.split(',');
for(var i = 0; i < jobid.length; i++)
if(jobid[i] !== ''){
get_sdelka(jobid[i], type_act)
}
//alert(type)
}


function get_sdelka(job_id,type_act){

if (confirm("Сохранить при этом результат?")) {
  $("#save_sdelka_inp").val("1");
}
...
// alert(type)
  geturl = $.ajax({
    type: "GET",
    url: '../count/table.php?type_act='+type_act+'&act=sum',
	data : '&year_num=<?=$year;?>&month_num=<?=$month;?>&num_sotr='+job_id,
    success: function () {
