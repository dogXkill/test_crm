...
<a href="?type=shtatnie&year=<?=$year;?>&month=<?=$month;?>" class="sublink" <? if($_GET['type'] == "shtatnie") {echo "style=\"font-weight:bold;border-bottom:none;\" ";} ?>>все штатные</a> |
...
<td class="table_title" style="width:100px">Сделка <img src="../../i/refresh.png" width="16" height="16" alt="" onclick="get_full_sdelka('get_sdelka')" style="cursor:pointer;"></td>
...
<td name="pay7_col" class="table_title" valign=top>Ордер <img src="../../i/refresh.png" width="16" height="16" alt="" onclick="get_full_sdelka('get_orders')" style="cursor:pointer;"></td>
...
if($type == "shtatnie"){$vstavka = " (proizv = '1' OR administration = '1') ";}
...
<td class=name><a href="/acc/users/users.php?edit=<?=$us['uid'];?>"><?;echo $fio;?></a>
...
<td align=center name="nachisleno_col"><input type="text" name="nachisleno" value="<?$nachisleno=$hrs[$job_id]["hours"]*$oklad_hour+$hrs[$us['job_id']]["boln"]*$work_time*$socoklad_hour-$progul_multa*$hrs[$job_id]["progul"]; echo $nachisleno;?>" id="nachisleno_<?=$job_id;?>"  class="general_inp" disabled/>
...
<br><span id="orders_<?=$job_id;?>" class="order_span"></span>
</td>
...
function get_full_sdelka(type){
job_ids = '<?=$job_ids_no_spec;?>';
jobid = job_ids.split(',');
for(var i = 0; i < jobid.length; i++)
if(jobid[i] !== ''){
get_sdelka(jobid[i], type)
}

}


function get_sdelka(job_id, type){
    if(type == "get_sdelka"){ $('#load_img_'+job_id).attr('src', '../../../../i/load.gif'); }
var geturl;
// alert(type)
  geturl = $.ajax({
    type: "GET",
    url: '../count/exp_html.php',
	data : '&year=<?=$year;?>&month=<?=$month;?>&act='+type+'&num_sotr='+job_id,
    success: function () {
