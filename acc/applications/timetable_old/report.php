...
//удаляем из строки последний символ, если это запятая
$act_ids = substr_replace($act_ids, '', strrpos($act_ids, ','));
//print_r($act_ids);
//получаем данные из уже сохраненной таблицы    report
$report = mysql_query("SELECT * FROM report WHERE year='$year' AND month='$month' GROUP BY uid");
while ($row = mysql_fetch_array($report)) {
$rpt[$row[1]]["work_time"] = $row[work_time];
$rpt[$row[1]]["oklad"] = $row[oklad];
$rpt[$row[1]]["socoklad"] = $row[socoklad];
$rpt[$row[1]]["sdelka"] = $row[sdelka];
$rpt[$row[1]]["procee"] = $row[procee];
$rpt[$row[1]]["pay1"] = $row[pay1];
$rpt[$row[1]]["pay1date"] = $row[pay1date];
$rpt[$row[1]]["pay2"] = $row[pay2];
$rpt[$row[1]]["pay2date"] = $row[pay2date];
$rpt[$row[1]]["pay3"] = $row[pay3];
$rpt[$row[1]]["pay3date"] = $row[pay3date];
$rpt[$row[1]]["pay4"] = $row[pay4];
$rpt[$row[1]]["pay4date"] = $row[pay4date];
$rpt[$row[1]]["pay5"] = $row[pay5];
$rpt[$row[1]]["pay5date"] = $row[pay5date];
$rpt[$row[1]]["pay6"] = $row[pay6];
$rpt[$row[1]]["pay6date"] = $row[pay6date];
$rpt[$row[1]]["pay7"] = $row[pay7];
$rpt[$row[1]]["pay7date"] = $row[pay7date];
$rpt[$row[pay]]["pay"]= $row[pay];
$rpt[$row[pay]]["paydate"]= $row[paydate];

if($working_days==""){$working_days= $row[working_days];}
}
...
<br><a href="report.php?year=<?=$current_year;?>&month=<?=$current_month;?>&type=<?=$type;?>" style="font-size:8px;">перейти в текущий месяц</a></td>
...
<a href="?type=nadomniki&year=<?=$year;?>&month=<?=$month;?>" class="sublink" <? if($_GET['type'] == "nadomniki") {echo "style=\"font-weight:bold;border-bottom:none;\" ";} ?>>надомники</a> |
<a href="?type=all&year=<?=$year;?>&month=<?=$month;?>" class="sublink" <? if($_GET['type'] == "all") {echo "style=\"font-weight:bold;border-bottom:none;\" ";} ?>>все</a>

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <a href="index.php?year=<?=$year;?>&month=<?=$month;?>&type=<?=$type;?>" target="_blank" class="sublink">отметить посещаемость</a>
...
<tbody>
<tr>
<td name="num_col" class="table_title" style="width:50px">#</td>
<td class="table_title" style="width:150px">ФИО
...
<td class="table_title" style="width:80px">База</td>
<td name="socoklad_col" class="table_title" style="width:80px">Соцбаза</td>
<td name="work_time_col" class="table_title" style="width:70px">Норма часов</td>

<td name="oklad_hour_col" class="table_title" style="width:70px">Рабочий час</td>
<td name="socoklad_hour_col" class="table_title" style="width:70px">Соцчас</td>
<td name="worked_time_col" class="table_title" style="width:70px">Отработано часов</td>
<td name="socnachisl_col" class="table_title" style="width:70px">Начислено соцчасов</td>
<td name="progul_col" class="table_title" style="width:70px">Прогулы</td>

<td class="table_title" style="width:100px">Начислено</td>
<td class="table_title" style="width:100px">Сделка <img src="../../i/refresh.png" width="16" height="16" alt="" onclick="get_full_sdelka()" style="cursor:pointer;"></td>
<td class="table_title" style="width:100px">Прочее</td>
<td class="table_title" style="width:100px;background-color: #DDFFCC;">Итого</td>
...
if($type == "administration"){$vstavka = " administration = '1'";}
if($type == "proizvodstvo" or !$type){$vstavka = " proizv = '1'";}
if($type == "nadomniki"){$vstavka = " nadomn = '1'";}
if($type == "all"){$vstavka = " (nadomn = '1' OR proizv = '1' OR administration = '1') ";}
if($working_days){
...
<td class=name><a href="/acc/users/users.php?edit=<?=$us['uid'];?>"><?$fio = $us['surname'].' '.$us['name'];echo $fio;?></a>
<a href="index.php?year=<?=$year;?>&month=<?=$month;?>&uid=<?=$us['uid'];?>&type=<?=$_GET['type'];?>" target="_blank"><img src="../../../i/timetable.png" width="16" height="16" alt=""></a>
</td>
...
<td align=center name="oklad_hour_col"><input type="text" name="oklad_hour" id="oklad_hour_<?=$job_id;?>" value="<?if($work_time >"0"){$oklad_hour = round($oklad/$work_time/$working_days); echo $oklad_hour;}?>" class="general_inp" disabled/></td>
<td align=center name="socoklad_hour_col"><input type="text" name="socoklad_hour" id="socoklad_hour_<?=$job_id;?>" value="<?if($work_time >"0"){$socoklad_hour = round($socoklad/$work_time/$working_days_social); echo $socoklad_hour;}?>" class="general_inp" disabled/></td>
...
<td align=center name="nachisleno_col"><input type="text" name="nachisleno" value="<?$nachisleno=$hrs[$job_id]["hours"]*$oklad_hour+$hrs[$us['job_id']]["boln"]*$work_time*$socoklad_hour-$progul_multa*$hrs[$job_id]["progul"]; echo $nachisleno;?>" id="nachisleno_<?=$job_id;?>"  class="general_inp" disabled/></td>
...
<span style="white-space: nowrap;word-wrap: normal;"><img src="../../i/refresh.png" width="16" height="16" alt="" onclick="get_sdelka('<?=$job_id;?>')" id="load_img_<?=$job_id;?>" style="cursor:pointer;">
<a href="../count/exp_html.php?year=<?=$year;?>&month=<?=$month;?>&num_sotr=<?=$job_id;?>&items_on_page=500" target="_blank"><img src="../../../i/table.png" width="16" height="16" alt="" id="table_sdelka_link_<?=$job_id;?>" style="display: <? if($sdelka > "0"){?>display: inline;<?}else{?>none<?}?>"></a>
</span></nobr>
</td>
...
<td align=center style="width:100px;background-color: #DDFFCC;"><input type="text" onkeyup="this.value=replace_num(this.value);" autocomplete="off" name="itogo" value="<?=$nachisleno+$sdelka+$procee;?>" id="itogo_<?=$job_id;?>"  class="general_inp" disabled/></td>

<td name="pay1_col">
<input type="checkbox" class="pay_sum" value="pay1_<?=$job_id;?>"/>
<input type="text" onkeyup="this.value=replace_num(this.value);" autocomplete="off" name="pay1"  value="<?$pay1=$rpt[$job_id]['pay1']; echo $pay1;?>" id="pay1_<?=$job_id;?>"  onchange="save_monthly_report('<?=$job_id;?>', 'pay1_<?=$job_id;?>')" class="pay_inp"/>
...
<td name="pay2_col">
<input type="checkbox" class="pay_sum" value="pay2_<?=$job_id;?>"/>
<input type="text" onkeyup="this.value=replace_num(this.value);" autocomplete="off" name="pay2"  value="<?$pay2=$rpt[$job_id]['pay2']; echo $pay2;?>" id="pay2_<?=$job_id;?>"  onchange="save_monthly_report('<?=$job_id;?>', 'pay2_<?=$job_id;?>')" class="pay_inp"/>
...
<td name="pay3_col">
<input type="checkbox" class="pay_sum" value="pay3_<?=$job_id;?>"/>
<input type="text" onkeyup="this.value=replace_num(this.value);" autocomplete="off" name="pay3"  value="<?$pay3=$rpt[$job_id]['pay3']; echo $pay3;?>" id="pay3_<?=$job_id;?>"  onchange="save_monthly_report('<?=$job_id;?>', 'pay3_<?=$job_id;?>')" class="pay_inp"/>
...
<td name="pay4_col">
<input type="checkbox" class="pay_sum" value="pay4_<?=$job_id;?>"/>
<input type="text" onkeyup="this.value=replace_num(this.value);" autocomplete="off" name="pay4"  value="<?$pay4=$rpt[$job_id]['pay4']; echo $pay4;?>" id="pay4_<?=$job_id;?>"  onchange="save_monthly_report('<?=$job_id;?>', 'pay4_<?=$job_id;?>')" class="pay_inp"/>
...
<td name="pay5_col">
<input type="checkbox" class="pay_sum" value="pay5_<?=$job_id;?>"/>
<input type="text" onkeyup="this.value=replace_num(this.value);" autocomplete="off" name="pay5"  value="<?$pay5=$rpt[$job_id]['pay5']; echo $pay5;?>" id="pay5_<?=$job_id;?>"  onchange="save_monthly_report('<?=$job_id;?>', 'pay5_<?=$job_id;?>')" class="pay_inp"/>
...
<td name="pay6_col">
<input type="checkbox" class="pay_sum" value="pay6_<?=$job_id;?>"/>
<input type="text" onkeyup="this.value=replace_num(this.value);" autocomplete="off" name="pay6"  value="<?$pay6=$rpt[$job_id]['pay6']; echo $pay6;?>" id="pay6_<?=$job_id;?>"  onchange="save_monthly_report('<?=$job_id;?>', 'pay6_<?=$job_id;?>')" class="pay_inp"/>
...
<td name="pay7_col">
<input type="checkbox" class="pay_sum" value="pay7_<?=$job_id;?>"/>
<input type="text" onkeyup="this.value=replace_num(this.value);" autocomplete="off" name="pay7"  value="<?$pay7=$rpt[$job_id]['pay7']; echo $pay7;?>" id="pay7_<?=$job_id;?>"  onchange="save_monthly_report('<?=$job_id;?>', 'pay7_<?=$job_id;?>')" class="pay_inp"/>
...
document.open('ved.php?administration='+administration+'&proizvodstvo='+proizvodstvo+'&nadomniki='+nadomniki+'&date='+date_ved,"","");
}
...
var geturl;
  geturl = $.ajax({
	...
	data : '&uid='+uid+'&year=<?=$year;?>&month=<?=$month;?>&work_time='+work_time+'&oklad='+oklad+'&socoklad='+socoklad+'&sdelka='+sdelka+'&procee='+procee+'&pay1='+pay1+'&pay1date='+pay1date+'&pay2='+pay2+'&pay2date='+pay2date+'&pay3='+pay3+'&pay3date='+pay3date+'&pay4='+pay4+'&pay4date='+pay4date+'&pay5='+pay5+'&pay5date='+pay5date+'&pay6='+pay6+'&pay6date='+pay6date+'&pay7='+pay7+'&pay7date='+pay7date+'&working_days='+working_days,
    success: function () {
...
var geturl;
  geturl = $.ajax({
    type: "GET",
    url: '../count/exp_html.php',
	data : '&year=<?=$year;?>&month=<?=$month;?>&act=get_sdelka&num_sotr='+job_id,
    success: function () {

var resp1 = geturl.responseText

if (resp1){
$("#sdelka_"+job_id).val(resp1);
